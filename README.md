...
- `main.js` — index-only choreography: chapter takeover observer, scroll reveals, parallax, specular glass highlights, the GhostDelta delta ticker, and Barry's pose cycle. All gated behind `prefers-reduced-motion`.
- `assets/` — app icons (512px), Barry the Banana poses, and marketing screenshots. Sourced from the app projects in `~/Documents/<AppName>`.
- `<app>/index.html`, `support.html`, `*PrivacyPolicy.html` — per-app pages on the shared system, each in its own brand palette.

## Adding an app

1. Add a 512px icon to `assets/icon-<slug>.png`.
